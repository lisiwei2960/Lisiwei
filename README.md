...
基于TimesNet的电力负荷预测系统，支持6小时、12小时和24小时预测。
...
1. 确保模型文件（TimeXer6.pth、TimeXer12.pth、TimeXer24.pth）已放置在checkpoints目录下
2. 上传的数据集格式需符合要求（具体格式见数据集示例）
3. 预测结果图片将保存在test_results目录下

## 常见问题

1. 如果遇到"No such file or directory"错误，请检查：
   - 模型文件是否存在于checkpoints目录
   - 数据集格式是否正确
   - 文件路径是否包含中文
